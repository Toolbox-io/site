...
class SupportChat {
    ...
    private async updateBotMessage(content: string, botMessageDiv: HTMLDivElement): Promise<void> {
        // Trim any whitespace and ensure clean content
        const cleanContent = content.trim();
        // Use regular parse to handle lists and other block elements
        botMessageDiv.innerHTML = await marked.parse(cleanContent);
                    ...
                    if (line.startsWith('data: ')) {
                        try {
                            ...
                            if (data.content) {
                                ...
                                await this.updateBotMessage(accumulatedText, botMessageDiv);
                            } else if (data.error) {
